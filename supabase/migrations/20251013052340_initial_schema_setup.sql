/*
  # Initial Database Schema Setup for Loan Application System
  ...
  1. user_profiles
    ...
    - `role` (text) - User role (merchant or admin)
    - `created_at` (timestamptz) - Record creation timestamp
    - `updated_at` (timestamptz) - Last update timestamp
  
  2. loans
    - Core loan application data with personal, contact, and financial details
    - Includes verification and disbursement tracking fields
    - Status tracking: Pending, Accepted, Rejected, Verified, Loan Disbursed
  
  3. loan_documents
    - Tracks uploaded documents for each loan
    - Links to loans table via loan_id
  
  ## Security
  - RLS enabled on all tables
  - Merchants can view/edit their own data
...
-- Create loans table with all fields including verification and disbursement
...
CREATE POLICY "Admins can insert documents for any loan"
  ON loan_documents FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM user_profiles WHERE id = auth.uid() AND role = 'admin')
