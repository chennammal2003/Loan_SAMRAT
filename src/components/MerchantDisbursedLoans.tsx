...
export default function MerchantDisbursedLoans() {
  ...
  const fetchLoans = async () => {
    ...
    try {
      ...
      console.error('Error fetching disbursed loans (merchant):', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    return loans.filter((l) => {
      const hay = [l.first_name, l.last_name, `${l.first_name} ${l.last_name}`, l.mobile_primary, l.email_id, l.address, l.id]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
  ...
  if (loans.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400">No loans have been disbursed for your account yet.</p>
      </div>
    );
  }
    ...
    <div>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {pageItems.map((loan) => (
                <tr key={loan.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{loan.first_name} {loan.last_name}</td>
                  <td className="px-6 py-4 text-sm font-medium text-purple-700 dark:text-purple-300">₹{loan.loan_amount.toLocaleString('en-IN')}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{loan.tenure} months</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{loan.mobile_primary}</td>
                  <td className="px-6 py-4">
                    <span className="inline-flex px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400">Loan Disbursed</span>
                  </td>
                  <td className="px-6 py-4">
                    <button
      ...
      {filtered.length > 0 && (
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1 rounded border bg-white dark:bg-gray-800 dark:border-gray-700 text-sm disabled:opacity-50">Prev</button>
            <span className="text-sm text-gray-600 dark:text-gray-300">Page {page} of {totalPages}</span>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1 rounded border bg-white dark:bg-gray-800 dark:border-gray-700 text-sm disabled:opacity-50">Next</button>
          </div>
        </div>
      )}
